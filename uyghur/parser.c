...
void Parser_buildLeaf(Parser *this, char tp, int num, Token *token, ...)
{
...
void Parser_consumeAstVariable(Parser *this)
{
    ...
    Parser_buildLeaf(this, UG_ATYPE_VAR, 2, name, token);
}
...
void Parser_consumeAstCommand(Parser *this)
{
    ...
    // 
    action->extra = args;
    Parser_buildLeaf(this, UG_ATYPE_CMD, 2, target, action);
}
...
void Parser_consumeAstConvert(Parser *this)
{
    ...
    if (Parser_isWord(this, 2, LETTER_MADE))
    {
        ...
        Parser_checkWord(this, 1, 1, LETTER_MADE);
        Parser_buildLeaf(this, UG_ATYPE_CVT, 2, target, source);
        return;
    }
...
void Parser_consumeAstJudge(Parser *this, char aType)
{
    ...
    if (!Parser_isType(this, 1, UG_TTYPE_CLC)) {
        ...
        Parser_buildLeaf(this, aType, 2, first, action);
        return;
    }
    // double
    ...
    Token *action = Parser_checkWord(this, 1, 2, LETTER_THEN, LETTER_ELSE);
    Parser_buildLeaf(this, aType, 4, second, clcltn, first, action);
}

void Parser_consumeAstIfFirst(Parser *this)
{
    // open UG_ATYPE_IF
    Parser_buildLeaf(this, UG_ATYPE_IF, 0, NULL);
    Parser_openBranch(this);
    // open UG_ATYPE_IF_F
...
void Parser_consumeAstIfLast(Parser *this)
{
    ...
    Parser_buildLeaf(this, UG_ATYPE_IF_L, 1, token);
    Parser_openBranch(this);
}
...
void Parser_consumeAstSpread(Parser *this)
{
    ...
    Parser_buildLeaf(this, UG_ATYPE_SPR, 3, iter2, iter1, target);
    Parser_openBranch(this);
}
...
bool _parser_mergeCalcuator(Queue *outputQueue, Token *operator) {
    Foliage *foliage = Foliage_new(operator);
    Object *right = Chain_pop_from_tail(outputQueue);
    Object *left = Chain_pop_from_tail(outputQueue);
    if (right == NULL || left == NULL) return false;
    foliage->right = right;
    foliage->left = left;
    Queue_push(outputQueue, foliage);
    return true;
}
...
void Parser_consumeAstCalculator(Parser *this)
{
    ...
    for (size_t i = 0; i < 1024; i++) {
        ...
        if (isValues || isTypes) {
            ...
            bool closeOk = false;
            while (operator != NULL) {
                if (is_eq_string(operator->value, SIGN_OPEN)) {
                    closeOk = true;
                    break;
                }
                bool isOk = _parser_mergeCalcuator(outputQueue, operator);
            ...
            Token *operator = Stack_pop(operatorStack);
            while (operator != NULL) {
                int currPeriority = get_calculation_priority(currToken->value);
                int periority = get_calculation_priority(operator->value);
                bool currHigher = currPeriority > periority;
                if (is_eq_string(operator->value, SIGN_OPEN) || currHigher) {
                    Stack_push(operatorStack, operator);
                    break;
                }
                bool isOk = _parser_mergeCalcuator(outputQueue, operator);
                Parser_assert(this, isOk, ERR_CALCULATOR);
...
void Parser_consumeAstGenerator(Parser *this)
{
    ...
    Parser_buildLeaf(this, UG_ATYPE_GNR, 2, target, body);  
}
